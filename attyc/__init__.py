    ...
    if type(classifier) == str:
        try:
            ...
            print(
                ...
                "Check if requested .py module in 'classifiers' directory is imported into '__init__.py'"
                " located in 'attyc' directory and that you use its alias defined in '__init__.py'."
                "\nEnd of program.")
            sys.exit(1)
...
def classify_atoms(input_sdf, classifier, file_output=False, screen_output=False):
    ...
    if file_output:
        ...
        io.create_output_file_for_parametrization(input_sdf, cl.get_atom_types(), classifier)
    if screen_output:
